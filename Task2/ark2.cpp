...
int main()
{
  ...
  while(temp<=1500 && rotation<=360)                               //Loop for checking different rotations of part image
    {

      temp=0;

      rotation = 0;
      Point2f src_center(part1.cols/2.0F, part1.rows/2.0F);           //Creating rotated image
      Mat rot_mat = getRotationMatrix2D(src_center, 45, 1.0);
      warpAffine(part1, part, rot_mat, part1.size());
      
      for( i=0;i<img.rows-part.rows;i++)                              //Loop for traversal of image
	{
	  for(j=0;j<img.cols-part.cols;j++)
	    {
	     
	      count=0;
	      if((abs(part.at<Vec3b>(0,70)[0]-img.at<Vec3b>(i,j+70)[0])<=e) && (abs(part.at<Vec3b>(0,70)[1]-img.at<Vec3b>(i,j+70)[1])<=e) && (abs(part.at<Vec3b>(0,70)[2]-img.at<Vec3b>(i,j+70)[2])<=e))       //checking if the 70th pixel from the current pixel matches the 70th pixel from the first pixel
		{
		 
		  for(l=0;l<part.rows;l++)                 //Loop for counting the no. of matched pixels
		    {
	      ...
	      if(count>=1500)                                   //Count thresholding
		{ 
		  temp=count;
		  X=i;
		  Y=j;
  ...
  A.x=Y;                                              //Drawing lines around detected section
  A.y=X;
  B.x=Y;
  B.y=X+part.rows;
  C.x=Y+part.cols;
